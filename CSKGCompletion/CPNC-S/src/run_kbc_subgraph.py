...
def main(args):
    ...
    test_graph = test_graph.to(torch.device('cuda:0'))
    ...
    if use_cuda and not args.no_cuda:
        ...
        model = model.cuda()
        #model.bert_concat_layer.embedding = model.bert_concat_layer.embedding.cpu()
    ...
    fusion_cluster_paraters = ['fc_fusion_R_C_hidden.weight', 'fc_fusion_R_C_hidden.bias', 'fc_fusion_T_C_hidden.weight', 'fc_fusion_T_C_hidden.bias']
    
    optimizer = torch.optim.Adam(
        [{'params': model.bert_concat_layer.parameters(), 'lr': 1e-4},
        {'params': model.rgcn.parameters(), 'lr': 1e-4},
        {'params': [p for n, p in model.decoder.named_parameters() if n in fusion_cluster_paraters], 'lr': 5e-5},
        {'params': [p for n, p in model.decoder.named_parameters() if n not in fusion_cluster_paraters], 'lr': 1e-4}],
        lr=args.lr
    )
    ...
    while True:
        ...
        for i in range(0, len(key_list), batch_size):
            ...
            graph_embeddings, tail_index = model.get_graph_embeddings(g, epoch)
            ...
            loss = model.get_score(e1, rel, target, graph_embeddings, epoch)
        ...
        if epoch % args.evaluate_every == 0:
            ...
            #if use_cuda and not args.no_cuda and cpu_eval:
            #    model.cpu()
            #    test_graph = test_graph.to(torch.device('cpu'))
            #    test_graph.ndata['id'] = test_graph.ndata['id'].cpu()
            #    test_graph.ndata['norm'] = test_graph.ndata['norm'].cpu()
            #    test_graph.edata['type'] = test_graph.edata['type'].cpu()
            #    model.decoder.no_cuda = True
            ...
            if use_cuda and not args.no_cuda and cpu_eval:
                model.cuda()
                model.bert_concat_layer.embedding = model.bert_concat_layer.embedding.cuda()
                test_graph = test_graph.to(torch.device('cuda:0'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--Concept_center_path",type=str, default=False,
                        help="concept embedding for each node")
