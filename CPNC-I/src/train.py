...
def main(args):
    ...
    if args.eval_only:
        ...
        if args.dataset == 'atomic' or args.dataset[:10] == 'conceptnet':
            ...
            model.cpu()
            g_whole = g_whole.to(torch.device('cpu'))
    ...
    if args.optimizer == "Adam":
        fusion_cluster_paraters = ['fc_fusion_H_C_hidden.weight', 'fc_fusion_H_C_hidden.bias', 'fc_fusion_T_C_hidden.weight', 'fc_fusion_T_C_hidden.bias']
        optimizer = torch.optim.Adam(
            [{'params': filter(lambda p: p.requires_grad, model.w_relation.parameters()), 'lr': args.lr},
            {'params': filter(lambda p: p.requires_grad, model.cluster_embedding.parameters()), 'lr': args.lr},
            {'params': filter(lambda p: p.requires_grad, model.encoder.parameters()), 'lr': args.lr},
            {'params': filter(lambda p: p.requires_grad, [p for n, p in model.decoder.named_parameters() if n in fusion_cluster_paraters]), 'lr': 5e-5},
            {'params': filter(lambda p: p.requires_grad, [p for n, p in model.decoder.named_parameters() if n not in fusion_cluster_paraters]), 'lr': args.lr}],
            lr=args.lr
        )
    ...
    while True:
        ...
        #current_graph = current_graph.to(torch.device('cuda:0'))
        ...
        if epoch % args.evaluate_every == 0:
            ...
            if args.dataset == 'atomic' or args.dataset[:10] == 'conceptnet':
                ...
                print('Transfer model back to:', args.device)
                model.to(args.device)
                g_whole = g_whole.to(torch.device('cuda:0'))
